...
Let `C_0` and `C_1` be two clusters of the data points. We assume that every data point is in either `C_0` or `C_1`. The cluster membership as being specified by `y`. We think of `y[i]` as being the label of point `i`. So, if `y[i] = 1`, then point `i` (and therefore row `i` of `A`) is an element of cluster `C_1`.  
...
Here's the trick: define a new vector `Z` such that:
...
3. While we're here, we will also check the identity `z^TD1 = 0`, where `1` is the vector of `n` ones (i.e. `np.ones(n)`). This identity effectively says that `z` should contain roughly as many positive as negative entries.
...
with respect to `z`, subject to the condition `z^TD1 = 0`.
...
We will construct the matrix `L = D^-1(D-A)`, which is often called the (normalized) *Laplacian* matrix of the similarity matrix `A`, and we will find the eigenvector corresponding to its second-smallest eigenvalue, naming it `z_eig`. Then, we will plot the data again, using the sign of `z_eig` as the color.
...
- Similarity Matrix A contains point connection information. It represents whether certain points are connected (1) or not (0).
  ...
  - There is symmetry accross diagonal signifying that points are connected recriprocally (e.g. 2 is connected to 4 and 4 is connected to 2)
