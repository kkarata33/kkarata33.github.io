...
def location_temp(country, year_begin, year_end, month, min_obs, **kwargs):
    ...
    df["avg"] = df.groupby(["NAME", "Month"])["Temp"].transform(np.mean) #applying linear regresison coef to df
    df = df.reset_index()
